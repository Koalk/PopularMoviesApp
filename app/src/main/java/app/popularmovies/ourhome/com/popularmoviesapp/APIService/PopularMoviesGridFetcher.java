...
import app.popularmovies.ourhome.com.popularmoviesapp.PosterGridFragment;
...
public class PopularMoviesGridFetcher extends AsyncTask<String,Void,List<MoviePoster>>{
    ...
    protected List<MoviePoster> doInBackground(String... params) {
        ...
        if (!AppConstants.API_SORT_FAVORITES.equals(sortField)) {
            ...
            Cursor favoriteMovies = moviesProvider.getMovieDetailByFavorite();
            if (favoriteMovies.moveToFirst()) {
                do {
                    moviePosterList.add(new MoviePoster(favoriteMovies.getString(1), null, favoriteMovies.getBlob(6)));
                }
                while (!favoriteMovies.isLast());
            }
        }
    ...
    private List<MoviePoster> getMoviesPosters(String movieListJsonStr)
        ...
        for(int i = 0; i < moviesArray.length(); i++) {
            ...
            MoviePoster moviePoster = new MoviePoster(movie.getString("id"), movie.getString("poster_path"), ImageUtils.getImageByteArray(movie.getString("poster_path")));
